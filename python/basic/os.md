...
| 功能             | 方法/属性                                          | 示例                                                                                                                                                           |
| ---------------- | -------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 获取脚本所在目录 | `os.path.dirname(__file__)`                        | `os.path.dirname(__file__)` → `C:\Users\jack`                                                                                                                  |
...
✅ os.path.dirname(**file**) 和 os.getcwd() 的核心区别

| 方法                      | 返回内容                                    | 适用场景                                   |
| ------------------------- | ------------------------------------------- | ------------------------------------------ |
| **os.getcwd()**           | **程序当前的工作目录（Working Directory）** | 你从哪里执行脚本，它就返回哪里             |
| **os.path.dirname(file)** | **当前这个 Python 文件所在的目录**          | 想根据脚本文件本身的位置找资源，应该用这个 |

✔ 一句话区别（最重要）

os.getcwd() → 你“在哪儿运行”脚本
